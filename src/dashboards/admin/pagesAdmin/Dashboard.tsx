...
import { getRecentActions, subscribe, type AuditEntry, logAction } from "../../../services/audit";
...
export default function Dashboard() {
    ...
    const [recent, setRecent] = useState<AuditEntry[]>(getRecentActions(6));

    useEffect(() => {
      const unsubscribe = subscribe(() => {
        setRecent(getRecentActions(6));
      });
      // initial refresh in case subscribe fires later
      setRecent(getRecentActions(6));
      return unsubscribe;
    }, []);
    ...
    return (
        <div className="space-y-3 sm:space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">
            <button onClick={() => { window.dispatchEvent(new CustomEvent('admin:navigate', { detail: { section: 'tableau' }})); logAction({ action: 'raccourci', entityType: 'navigation', entityId: 'tableau', metadata: { label: 'Tableau' } }); }} className="bg-white rounded-lg shadow-sm border border-gray-200 p-2 hover:shadow-md transition-shadow flex items-center space-x-2 cursor-pointer text-left">
              <div className="p-1 rounded-md bg-blue-100">
                <LayoutDashboard className="w-4 h-4 text-blue-600" />
              </div>
              <span className="text-xs font-medium text-gray-800">Tableau</span>
            </button>
            <button onClick={() => { window.dispatchEvent(new CustomEvent('admin:navigate', { detail: { section: 'utilisateurs' }})); logAction({ action: 'raccourci', entityType: 'navigation', entityId: 'utilisateurs', metadata: { label: 'Utilisateurs' } }); }} className="bg-white rounded-lg shadow-sm border border-gray-200 p-2 hover:shadow-md transition-shadow flex items-center space-x-2 cursor-pointer text-left">
              <div className="p-1 rounded-md bg-emerald-100">
                <Users className="w-4 h-4 text-emerald-600" />
              </div>
              <span className="text-xs font-medium text-gray-800">Utilisateurs</span>
            </button>
            <button onClick={() => { window.dispatchEvent(new CustomEvent('admin:navigate', { detail: { section: 'documents' }})); logAction({ action: 'raccourci', entityType: 'navigation', entityId: 'documents', metadata: { label: 'Documents' } }); }} className="bg-white rounded-lg shadow-sm border border-gray-200 p-2 hover:shadow-md transition-shadow flex items-center space-x-2 cursor-pointer text-left">
              <div className="p-1 rounded-md bg-purple-100">
                <FileText className="w-4 h-4 text-purple-600" />
              </div>
              <span className="text-xs font-medium text-gray-800">Documents</span>
            </button>
            <button onClick={() => { window.dispatchEvent(new CustomEvent('admin:navigate', { detail: { section: 'api' }})); logAction({ action: 'raccourci', entityType: 'navigation', entityId: 'api', metadata: { label: 'API' } }); }} className="bg-white rounded-lg shadow-sm border border-gray-200 p-2 hover:shadow-md transition-shadow flex items-center space-x-2 cursor-pointer text-left">
            ...
            <button onClick={() => { window.dispatchEvent(new CustomEvent('admin:navigate', { detail: { section: 'email' }})); logAction({ action: 'raccourci', entityType: 'navigation', entityId: 'email', metadata: { label: 'Emails' } }); }} className="bg-white rounded-lg shadow-sm border border-gray-200 p-2 hover:shadow-md transition-shadow flex items-center space-x-2 cursor-pointer text-left">
              <div className="p-1 rounded-md bg-orange-100">
                <Mail className="w-4 h-4 text-orange-600" />
              </div>
              <span className="text-xs font-medium text-gray-800">Emails</span>
            </button>
            <button onClick={() => { window.dispatchEvent(new CustomEvent('admin:navigate', { detail: { section: 'parametres' }})); logAction({ action: 'raccourci', entityType: 'navigation', entityId: 'parametres', metadata: { label: 'Paramètres' } }); }} className="bg-white rounded-lg shadow-sm border border-gray-200 p-2 hover:shadow-md transition-shadow flex items-center space-x-2 cursor-pointer text-left">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-3 sm:p-4">
              <div className="flex items-center justify-between mb-2 sm:mb-3">
                ...
                <div className="inline-flex items-center space-x-1 text-[10px] text-gray-500">
                  ...
                  <span>{recent.length} entrées</span>
                </div>
              </div>
              <div className="space-y-2 sm:space-y-3">
                {recent.length === 0 ? (
