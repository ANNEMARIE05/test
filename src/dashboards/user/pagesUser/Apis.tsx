              ...
              <div key={apiKey.id} className="border border-gray-200 rounded-lg overflow-hidden">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-1.5">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-2 space-y-1 sm:space-y-0">
                          <div className="flex items-center space-x-1 sm:space-x-0 sm:flex-col sm:items-start">
                            <p className="text-xs text-gray-600">Clé: <code className="bg-gray-100 px-1 py-0.5 rounded break-all text-xs">{apiKey.key}</code></p>
                            <button
                        ...
                        <p className="text-xs text-gray-600">Créée le: {apiKey.createdAt}</p>
                      </div>
                      ...
                      <div className="mt-1.5">
                        <div className="flex justify-between items-center mb-1">
                          ...
                          <span className={`text-xs font-medium ${getQuotaColor(getQuotaPercentage(apiKey.quota.used, apiKey.quota.limit))}`}>
                        ...
                        <p className="text-xs text-gray-500 mt-0.5">Réinitialisation le {apiKey.quota.resetDate}</p>
                      </div>
                    </div>
                ...
                {expandedKeys.has(apiKey.id) && (
                  <div className="border-t border-gray-200 bg-gray-50 p-2">
                    <h5 className="text-xs font-medium text-gray-900 mb-1.5">Logs d'utilisation - {apiKey.name}</h5>
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                              {usageLogs.map(log => (
                                <tr key={log.id}>
                                  ...
                                  <td className="px-2 py-1 text-xs text-gray-500">{log.responseTime}</td>
                                  <td className="px-2 py-1 text-xs text-gray-500">{log.quotaUsed}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
